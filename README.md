...
Предсказание количества долгов — задача линейной регрессии, цель которой — установить связь между $`X`$ (множество всех оценок студента) и $`Y=R_{\geq 0}`$ (множество неотрицательных действительных чисел, показывающее количество неудовлетворительных оценок студента).
