...
Предсказание количества долгов — задача линейной регрессии, цель которой — установить связь между $X$ (множество всех оценок) и $Y$ (множество количества двоек).

Пусть также задана функция потерь $L(y,\hat{y}): Y \times Y \rightarrow R$, которая показывает цену ошибки, если при правильном ответе $y$ был предсказан $\hat{y}$. Мы будем рассматривать среднеквадратичную функцию потерь: $$L(y,\hat{y}) = \sqrt{\sum\limits_{i=1}^{n} (y - \hat{y})^2 \over {n}}$$
**Входные данные**: $X$ — множество всех оценок, $Y$ — множество количества двоек.
**Модель**: $M(X,P)$, $P$ — набор параметров модели. В нашем случае используется модель линейной регрессии с использованием метода градиентного бустинга.
Он создает более сильную модель путем последовательного добавления слабых моделей (деревьев решений), каждая из которых исправляет ошибки предыдущей, что позволяет минимизировать функцию потерь.
...
- добавлен столбец с семестром каждого предмета (курс = учебный год - год группы + 1, семестр = курс * 2 - 1 (1 полугодие) или курс * 2 (2 полугодие))
...
Гиперпараметры для каждой модели были подобраны с помощью библиотеки optuna (итерационный алгоритм TPE — Tree-structured Parzen Estimator), минимизируя среднеквадратичную ошибку.
